...
// Unity 6.3 起资源打开回调使用 EntityId，避免继续依赖旧的整数实例 ID。
...
namespace Animancer.Editor.TransitionLibraries
{
        ...
        SerializedDataEditorWindow<TransitionLibraryAsset, TransitionLibraryDefinition>
    {
        ...
        /// </summary>
        [OnOpenAsset]
        private static bool OnOpenAsset(ObjectID objectID, int line)
        {
#if UNITY_6000_3_OR_NEWER
            var library = EditorUtility.EntityIdToObject(objectID) as TransitionLibraryAsset;
#else
            var library = EditorUtility.InstanceIDToObject(objectID) as TransitionLibraryAsset;
#endif
            if (library == null)
                return false;
